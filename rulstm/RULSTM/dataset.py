...
#         if dd is None:
#             print(f"Frame {f} not found in LMDB database.")
#             continue
        
#         # 解压缩并将数据转换为 numpy 数组
#         decompressed_data = zlib.decompress(dd)
#         data = np.frombuffer(decompressed_data, dtype=np.float16)  # 使用 float16 作为数据类型
...
class SequenceDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f"out: {out['past_features'].shape}")
