...
def read_representations(frames, env, tran=None):
    ...
    for f in frames:
        ...
        data = np.frombuffer(dd, 'float16').reshape(197, 192).astype('float32')
        # print(f"Loaded data shape: {data.shape}")  # 输出检查

        features.append(data)
    # convert list to numpy array
    # 先将 features 转换为一个 numpy 数组，然后再转换为 torch.Tensor
    features = np.array(features)  # 将列表转换为 numpy 数组
    features = torch.tensor(features, dtype=torch.float32)  # 转换为 PyTorch Tensor
