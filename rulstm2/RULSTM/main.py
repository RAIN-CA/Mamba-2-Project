...
def get_loader(mode, override_modality = None):
    if override_modality:
        ...
        path_to_lmdb = join(args.path_to_data, "lmdb")
...
def get_model():
    if args.modality != 'fusion':  # single branch
        model = MambaTimeSeriesClassifier_V3(args.num_class, args.feat_in, args.hidden,
                       args.dropout)
        # model = RUMamba(args.num_class, args.feat_in, args.hidden,
        #                args.dropout)
        ...
        for mode in ['training', 'validation']:
            ...
            with torch.set_grad_enabled(mode == 'training'):
                ...
                for i, batch in enumerate(loaders[mode]):
                    ...
                    # print(f"linear_labels: {linear_labels}")
                    # print(f"linear_preds: {linear_preds}")
                    

                    loss = F.cross_entropy(linear_preds, linear_labels)
                    # print(f"class loss: {loss}")
                    # print(f"mamba loss: {model.get_mamba_loss()}")
                    # loss += model.get_mamba_loss()
                    ...
                    # # 释放不再使用的变量，避免显存占用
                    del x, y, preds, linear_preds, linear_labels, loss
...
def get_validation_ids():
    unseen_participants_ids = pd.read_csv(
        join(args.path_to_data, 'validation_unseen_participants_ids.csv'),
        names=['id']
    )['id']

    tail_verbs_ids = pd.read_csv(
        join(args.path_to_data, 'validation_tail_verbs_ids.csv'),
        names=['id']
    )['id']

    tail_nouns_ids = pd.read_csv(
        join(args.path_to_data, 'validation_tail_nouns_ids.csv'),
        names=['id']
    )['id']

    tail_actions_ids = pd.read_csv(
        join(args.path_to_data, 'validation_tail_actions_ids.csv'),
        names=['id']
    )['id']
