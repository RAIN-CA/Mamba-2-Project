...
def simple_collate_fn(batch, window_size=10, future_offset=1):
    """
    自定义 collate_fn，直接根据 Dataset 返回的帧进行窗口分割，并获取未来帧作为标签。
    
    参数:
    - batch: 输入的批次数据
    - window_size: 每个序列的窗口大小
    - future_offset: 未来帧的偏移量，决定标签应该是未来的第几帧
    """
    sequences = []
    labels = []
    future_frames = []
    ...
    for data in batch:
        ...
        # 如果帧数量不足以提取指定长度的窗口和未来帧，则跳过
        if num_frames < window_size + future_offset:
            print(f"跳过特征数量不足的样本，帧数量: {num_frames}")
            continue
        
        # 直接分割特征，按顺序读取
        for i in range(0, num_frames - window_size - future_offset + 1):
            sequence = frame_features[i:i + window_size]
            future_frame = frame_features[i + window_size + future_offset - 1]
            sequence_labels = frame_labels[i:i + window_size]
            future_label = frame_labels[i + window_size + future_offset - 1]
...
eval_dataset = VideoActionDataset(
    ...
    frame_interval=30,         # 每隔30帧提取一帧
    frame_segment_length=30,   # 每次返回的帧片段长度为10
    sequence_length=20,
    model=internvl_model,
    image_processor=image_processor
)
...
eval_dataloader = DataLoader(
    ...
    shuffle=False,  # 不打乱数据
    collate_fn=lambda x: simple_collate_fn(x, window_size)  # 保留之前的过滤逻辑
)
...
def run_evaluation(model, model_path, eval_dataloader, device='cuda', result_dir='/root/autodl-tmp/mamba_results/eval_result'):
    ...
    # 评估模型
    grouped_evaluation_with_model(eval_dataloader, model, 1,  device)
...
# 示例使用
run_evaluation(mamba_2_query_model, '/root/autodl-tmp/mamba_results/pth/model_seq20_epoch_3.pth', eval_dataloader)
